...
TEST(SignedQuantizedReadAssignVariable, TwoSubgraphsReadAssign) {
...
TEST(SignedQuantizedReadAssignVariable, TwoSubgraphsReadAssignOneVarHandle) {
  auto xnnpack_delegate = NewXnnPackDelegateSupportingVariableOps();
  TfLiteDelegate* delegate = xnnpack_delegate.get();
  std::random_device random_device;
  auto rng = std::mt19937(random_device());
...
TEST(SignedQuantizedReadAssignVariable, TwoSubgraphsReadAssignOneVarHandle2) {
  auto xnnpack_delegate = NewXnnPackDelegateSupportingVariableOps();
  TfLiteDelegate* delegate = xnnpack_delegate.get();
  std::random_device random_device;
  auto rng = std::mt19937(random_device());
