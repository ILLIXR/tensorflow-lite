...
#include "delegates/xnnpack/quantized_variable_ops_tester.h"

namespace tflite {
namespace xnnpack {

TEST(UnsignedQuantizedReadAssignVariable, SimpleAssignThenRead) {
...
TEST(UnsignedQuantizedReadAssignVariable, AssignTwiceThenRead) {
  auto xnnpack_delegate = NewXnnPackDelegateSupportingVariableOps();
  TfLiteDelegate* delegate = xnnpack_delegate.get();
  std::random_device random_device;
  auto rng = std::mt19937(random_device());
...
TEST(UnsignedQuantizedReadAssignVariable,
...
TEST(UnsignedQuantizedReadAssignVariable, TwoVarHandlesAssignThenRead) {
  auto xnnpack_delegate = NewXnnPackDelegateSupportingVariableOps();
  TfLiteDelegate* delegate = xnnpack_delegate.get();
  std::random_device random_device;
  auto rng = std::mt19937(random_device());
