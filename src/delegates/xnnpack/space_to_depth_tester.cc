...
#include "delegates/xnnpack/space_to_depth_tester.h"
...
#include "model.h"
#include "schema/schema_conversion_utils.h"
#include "version.h"

namespace tflite::xnnpack {

template <class T>
void SpaceToDepthTester::Test(TensorType tensor_type,
...
template <>
void SpaceToDepthTester::Test<float>(TensorType tensor_type,
                                     Interpreter* delegate_interpreter,
                                     Interpreter* default_interpreter) const {
  std::random_device random_device;
  auto rng = std::mt19937(random_device());
...
std::vector<char> SpaceToDepthTester::CreateTfLiteModel(
    TensorType tensor_type) const {
  flatbuffers::FlatBufferBuilder builder;
  const flatbuffers::Offset<OperatorCode> operator_code =
      CreateOperatorCode(builder, BuiltinOperator_SPACE_TO_DEPTH, 0);
  ...
  const flatbuffers::Offset<Operator> op = CreateOperator(
      ...
      CreateSpaceToDepthOptions(builder, BlockSize()).Union());
  ...
  const flatbuffers::Offset<Model> model_buffer = CreateModel(
      ...
      builder.CreateString("Space-To-Depth model"),
